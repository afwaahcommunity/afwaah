...
export function createRedisConnection(env: RealtimeEnv): RedisConnection {
  const raw = createRedisClient(env);
  const pub = createRedisClient(env);
  const sub = createRedisClient(env);

  return {
    client: new IoredisClient(raw),
    async close() {
      await Promise.all([safeQuit(raw), safeQuit(pub), safeQuit(sub)]);
    },
    async healthCheck() {
      await raw.ping();
    },
