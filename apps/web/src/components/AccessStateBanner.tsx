...
export function AccessStateBanner({
      ...
      <div className="flex items-start gap-2.5 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2.5 text-sm">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
            ...
            <Link href="/blocked" className="text-primary hover:underline">Details</Link>
          </div>
        </div>
      </div>
    );
  ...
  if (write && write.kind !== "allowed") {
    const fallback: { label: string; hint?: string } = { label: "Location check failed." };
    ...
    const m = map[write.kind] ?? fallback;
    return (
      <div className="flex items-start gap-2.5 rounded-md border border-border bg-muted/40 px-3 py-2.5 text-sm">
        ...
        <div className="min-w-0 flex-1">
          ...
          {write.kind !== "loading" && (
            <Link href="/verify-location" className="mt-1 inline-block text-xs text-primary hover:underline">
              Verify location →
            </Link>
          )}
        </div>
