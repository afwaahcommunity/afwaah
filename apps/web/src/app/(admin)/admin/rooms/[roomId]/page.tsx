...
import type { BanState, Report } from "@/lib/types";
...
export default function AdminRoomDetailPage() {
  ...
  const messagesQ = useQuery({
    ...
    queryFn: () => api.admin.roomMessages(roomId),
  });
  ...
  const submitBan = async (b: Omit<BanState, "roomId"> & { roomId?: string }) => {
    if (!banTarget) return;
  ...
  return (
    <AdminShell>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Stat
          label="Open reports"
          value={roomReports.filter((r) => r.status === "open").length}
          tone={roomReports.some((r) => r.status === "open") ? "warn" : "muted"}
        />
...
function ReportsSection({ reports }: { reports: Report[] }) {
  return (
    <Section title="Room reports" action={<span className="text-xs text-muted-foreground">{reports.length} total</span>}>
      {reports.length === 0 ? (
...
function MessagesSection({
  ...
  messages: { id: string; displayName: string; displayColor: string; content: string; createdAt: string }[];
}) {
                ...
                {openMenu === p.userId && (
                  <>
                    ...
                    <div className="absolute right-0 z-20 mt-1 w-44 overflow-hidden rounded-md border border-border bg-popover text-sm shadow-lg">
                      <Link href={`/admin/users/${p.userId}`} className="block px-3 py-2 hover:bg-accent">
                        View profile
                      </Link>
                      <button
                        onClick={() => {
