...
function PrimaryAction({ primary }: { primary: Details["primary"] }) {
  ...
  return (
    <Link href={primary.to ?? "/rooms"} className={`${base} bg-primary text-primary-foreground`}>
      {primary.label}
    </Link>
  );
}
  ...
  switch (state) {
    ...
    case "room_ban":
      return {
        ...
        summary: "You cannot read or join this specific room. Every other room stays available.",
