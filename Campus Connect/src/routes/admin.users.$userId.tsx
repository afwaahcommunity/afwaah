...
import { timeAgo, shortTime } from "@/lib/time";
import { toast } from "sonner";
import { AlertTriangle, ArrowLeft, Ban, Flag, MessageSquare } from "lucide-react";
import type { ReportReason } from "@/lib/types";

export const Route = createFileRoute("/admin/users/$userId")({
  head: () => ({
    meta: [{ title: "User — admin" }, { name: "robots", content: "noindex" }],
  }),
  component: UserDetail,
  errorComponent: ({ error, reset }) => (
    <AdminShell>
      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4 text-sm">
        <p className="font-medium text-destructive">Couldn't load user</p>
...
const labelReason = (r: string) =>
  (REASON_LABELS as Record<string, string>)[r] ?? r;
...
function UserDetail() {
  ...
  const messagesQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const lists = await Promise.all(rooms.map((r) => api.messages.list(r.id)));
      ...
      <AdminShell>
        ...
        <div className="mt-4 rounded-lg border border-border bg-card px-4 py-8 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            The user id <span className="font-mono">{userId}</span> doesn't exist or was removed.
          </p>
        </div>
      </AdminShell>
    );
  ...
  return (
    <AdminShell>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                <span className="font-mono">{user.id}</span> · joined {timeAgo(user.createdAt)} · last seen {timeAgo(user.lastSeenAt)}
              </p>
            </div>
          </div>
          <button
      ...
      <BanActionDialog
        ...
        onSubmit={async (b) => {
          await api.admin.banUser({ userId: user.id, kind: b.kind, reason: b.reason, expiresAt: b.expiresAt });
...
// AlertTriangle import kept in case future error surfaces need it inline
void AlertTriangle;
