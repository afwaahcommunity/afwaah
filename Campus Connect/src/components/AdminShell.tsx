...
import { Flag, Home, LayoutGrid, LogOut, Users } from "lucide-react";
import { clearAdminSession, loadAdminSession } from "@/lib/session";
import { useNavigate } from "@tanstack/react-router";

const nav: { to: "/admin" | "/admin/reports" | "/admin/rooms"; label: string; icon: typeof Home; exact?: boolean }[] = [
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            <span className="h-2 w-2 rounded-full bg-warning" style={{ backgroundColor: "var(--warning)" }} />
            campus / admin
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline text-xs text-muted-foreground">{admin?.name ?? "not signed in"}</span>
            <ThemeToggle />
            <button
              onClick={() => { clearAdminSession(); navigate({ to: "/admin/login" }); }}
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 px-4 py-6 md:grid-cols-[12rem_1fr]">
        <aside className="md:sticky md:top-20 md:self-start">
          <nav className="flex gap-1 overflow-x-auto md:flex-col md:overflow-visible">
            ...
            <Link
              to="/rooms"
              className="inline-flex items-center gap-2 whitespace-nowrap rounded-md px-2.5 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground"
            >
